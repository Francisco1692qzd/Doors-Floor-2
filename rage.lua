...
task.spawn(function()
	...
	local rawURL = "https://github.com/Francisco1692qzd/Doors-Floor-2/blob/main/rage.rbxm?raw=true"
	local entity = nil
	...
	if currentRoom then
		moduleScripts.Module_Events.flicker(currentRoom, 6)
	end
		...
		while entity ~= nil and pr ~= nil and entity.Parent ~= nil do wait(0.8)
			...
			if v.Character ~= nil and v.Character.HumanoidRootPart ~= nil then
				if canSeeTarget(v.Character, 45) and not v.Character:GetAttribute("Hiding") then
					...
					local idImg = "rbxassetid://11235968586"
					...
					backgroundmommy.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
					...
					image.ZIndex = 6 -- Puts image in front of background

					local jumpscareNoise = pr.Scream.SoundId
					...
					firesignal(remotesFolder.DeathHint.OnClientEvent, {"You died to who you call Rage...", "His presence is known by rebound and increasing its limit speed..", "Hide or search for a safe spot!"}, "Blue")
