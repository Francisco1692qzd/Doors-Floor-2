...
task.spawn(function()
	...
	if currentRoom then
		moduleScripts.Module_Events.flicker(currentRoom, 4)
	end
		...
		while entity ~= nil and pr ~= nil and entity.Parent ~= nil do wait(0.3)
			...
			if v.Character ~= nil and v.Character.HumanoidRootPart ~= nil then
				if canSeeTarget(v.Character, 45) and not v.Character:GetAttribute("Hiding") then
					...
					firesignal(remotesFolder.DeathHint.OnClientEvent, {"You died to Trauma...", "His presence is known when you hear him behind the next door", "Hide or seek for a safe spot!"}, "Blue")
				end
			end
